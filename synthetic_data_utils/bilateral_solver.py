...
grid_params = {
    'sigma_luma': 1,  # Brightness bandwidth
    'sigma_chroma': .5,  # Color bandwidth
    'sigma_spatial': 1  # Spatial bandwidth
}
...
def apply_bilateral(reference, target, confidence, thresh=0.7, plot=False):
    ...
    output_solver = ((output_solver - np.min(output_solver)) /
                     (np.max(output_solver) - np.min(output_solver)))
    ...
    if plot:
        ...
        ax1.set_title('Input')
        ax1.axis('off')
        ax2.imshow(cv2.bitwise_and(
            reference, reference,
            mask=np.array(t.reshape(im_shape) > 0, dtype=np.uint8)))
